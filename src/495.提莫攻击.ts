...
function findPoisonedDuration(timeSeries: number[], duration: number): number {
  // 解法1：遍历时间，如果与下一个的时间差大于间隔，则增加间隔时间的中毒；如果小于，则增加两个时间差
  return timeSeries.reduce((tAll, tNow, i) => {
    let tNext = timeSeries[i + 1] ?? Number.MAX_SAFE_INTEGER;
    return tAll + Math.min(duration, tNext - tNow);
  }, 0);
}
// @lc code=end
